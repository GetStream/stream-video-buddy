...
async function getBrowser(options) {
  return await chromium.launch({
    ...
    args: ["--disable-dev-shm-usage"] // This makes Chrome more stable on CI
  });
}
...
function ensureEnvVars() {
  const app_url = process.env.STREAM_SDK_TEST_APP ?? ""
  const email = process.env.STREAM_SDK_TEST_ACCOUNT_EMAIL ?? ""
  const password = process.env.STREAM_SDK_TEST_ACCOUNT_PASSWORD ?? ""

  if (app_url == null || email == null || password == null) {
    throw "Stream SDK app url, email and password should be provided."
  }
}
